...
def hybrid_noFR(configdict):
    ...
    # RANDOM: k3 random Queries
    #---------------------------------------------------------------------------------------------------------------------------------------
    bidder_names = list(ELICITED_BIDS.keys())
    logging.warning('RANDOM: k3:{} random Queries'.format(k3))
    NEW_BIDS = random_bids_mlca_unif(SATS_auction_instance=SATS_auction_instance,
                                     number_random_bids=k3, bidder_names=bidder_names,
                                     fitted_scaler=mlca1[1]['Fitted Scaler'])
    ...
    EFFICIENCY_PER_ITER['RANDOM_k3'] = E.efficiency_per_iteration
    del E
