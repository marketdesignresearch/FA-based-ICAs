...
This file implements the class Economies. This class is used for simulating MLCA, i.e., the MLCA mechanism, based on DNNs (Deep Neural Networks).
Specifically, this class stores all the information about the economies in the iterative procedure of the DNN-based preference elicitation algorithm.
...
from mlca.mlca_nn_mip import MLCA_NNMIP
...
class MLCA_Economies:
    ...
    def get_info(self, final_summary=False):
        ...
        if (not final_summary) and (not len(self.efficiency_per_iteration.keys())==0): logging.warning('Efficiency given elicited bids from iteration 0-%s: %s\n', self.mlca_iteration-1,self.efficiency_per_iteration[self.mlca_iteration-1])
    ...
    def set_initial_bids(self, initial_bids=None, fitted_scaler=None, seed=None):
        ...
        if initial_bids is None: # Uniform sampling (now with correct isLegacy Version)
            ...
            if not (list(initial_bids.keys()) == self.bidder_names):
                logging.info('Cannot set inputed initial bids-> sample uniformly.') # Uniform sampling (now with correct isLegacy Version)
                self.elicited_bids, self.fitted_scaler = util.initial_bids_mlca_unif(SATS_auction_instance=self.SATS_auction_instance,
                                                                                 number_initial_bids=self.Qinit,bidder_names=self.bidder_names,
                                                                                 scaler=self.scaler,seed=seed)
            else:
